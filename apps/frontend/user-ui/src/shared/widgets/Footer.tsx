        ...
        <div className="footer-part2">
          ...
          {categories.map((i, index) => (
            <div key={index} className="footer-category">
              {/* ✅ UPDATED: Redirects to Shop page with the Category filter */}
              <Link href={`/shop?category=${encodeURIComponent(i.title)}`}>
                <p>{i.title}</p>
              </Link>
            </div>
          ))}
        ...
        <div className="footer-part3">
          ...
          {pages.map((i, index) => (
            <div key={index} className="footer-pages">
              {/* Pages usually have specific routes, so we keep i.href here */}
              <Link href={i.href}>
                <p>{i.title}</p>
              </Link>
            </div>
