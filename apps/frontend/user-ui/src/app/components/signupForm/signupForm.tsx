...
// const provider = new GoogleAuthProvider();
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      await axios.post(
        `https://userservice.zeabur.app/api/auth/register/otp/initiate`,
        { email: enteredEmail }
      );
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      await axios.post(
        `https://userservice.zeabur.app/api/auth/register/otp/verify`,
        {
          email,
          otp: otpCode,
        }
...
const onSubmit = async (data: FormData) => {
  ...
  try {
    await axios.post(
      `https://userservice.zeabur.app/api/auth/register/otp/complete`,
      {
        email,
 ...
 const loginResponse = await axios.post(
  `https://userservice.zeabur.app/api/auth/login`,
  {
...
// ✅ Redirect after login
  ...
  const resendOtp = async () => {
    ...
    setLoading(true);
    try {
      await axios.post(`https://userservice.zeabur.app/api/auth/register/otp/initiate`, {
        email,
      });
      setCanResend(false);
      setTimer(60);
...
router.push('/myaccount');
  ...
  return (
    <div className="login-page signupform">
