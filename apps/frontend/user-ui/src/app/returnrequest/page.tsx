...
const [loadingReturnRequests, setLoadingReturnRequests] = useState(true);
...
useEffect(() => {
  async function fetchReturnRequests() {
    ...
    try {
      ...
      setReturnRequestError(err.message || 'Failed to load return requests');
    } finally {
...
useEffect(() => {
    async function fetchPopularProducts() {
      try {
        const res = await fetch(`http://productservice.zeabur.app/api/products`);
...
async function handleDownloadInvoice(orderId: string) {
  try {
    ...
    const res = await fetch(`http://invoiceservice.zeabur.app/api/invoices/${orderId}/download`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
...
const handleCancelOrder = async (order: OrderData) => {
  ...
  try {
    const API_URL = 'http://orderservice.zeabur.app'; 
  ...
  <div className="orderapagefull">
    <div className="orderpageleft-2">
      <div className="yourorder-header">
        <h2 className="sectiontitle">Your Returns/Replace</h2>
...
>
  ...
  {loadingOrders ? (
    ...
    (() => {
      const returnRequestedOrders = filteredOrders.filter(order =>
        returnRequests.some(req => req.orderId === order.id)
      );

      return returnRequestedOrders.length > 0 ? (
        returnRequestedOrders.map(order => (
          <div key={order.id} className="sectionsorder">
            <ReturnCard
              onDownloadInvoice={handleDownloadInvoice}
              order={order}
              onCancelOrder={(order) => handleCancelOrder(order as any)}
            />
            <ReturnRequest
              orders={[order]}
              loading={false}
        ...
        <p className="empty-message">No return requested orders found.</p>
      );
    })()
  )}
</div>
