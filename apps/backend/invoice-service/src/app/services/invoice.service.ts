...
export const invoiceService = {
  ...
  const items: InvoiceItem[] = order.items.map((item: typeof order.items[number]) => ({
  description: item.product.title,
  unitPrice: item.unitPrice instanceof Decimal ? item.unitPrice.toNumber() : item.unitPrice,
